...
const postApprove = async (id) => {
  ...
  if (remarks) {
    await confirmApproval(id, remarks);
    $swal.fire("Approved!", "The request has been approved.", "success");
  }
};

const postReject = async (id) => {
  const { value: remarks } = await $swal.fire({
    title: "Are you sure?",
  ...
  if (remarks) {
    await rejectApproval(id, remarks);
