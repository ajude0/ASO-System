    ...
    <div class="bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[95vh] flex flex-col m-4">
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-80 border-r bg-gray-50 p-4 overflow-y-auto">
          ...
          <div class="bg-white rounded-lg shadow p-4 mb-4">
            ...
            <div v-show="showAddForm" class="space-y-3 pt-2 border-t">
              <div>
                ...
                <select v-model="newBoxForm.assignedTo" class="w-full border rounded px-3 py-2 text-sm">
              ...
              <div class="flex items-center gap-2">
                ...
                <label for="hasDate" class="text-sm font-medium text-gray-700">Include date field</label>
              </div>

              <div v-if="newBoxForm.hasDate">
                <label class="block text-sm font-medium text-gray-700 mb-1">Default Date Offset</label>
                <input v-model.number="newBoxForm.dateOffset" type="number" min="0" class="w-full border rounded px-3 py-2 text-sm" />
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow p-4">
             ...
             <div class="space-y-2 max-h-96 overflow-y-auto">
              <div
                v-for="(box, index) in signatureBoxes"
                :key="box.id"
              ...
              >
                ...
                <div class="text-xs text-gray-500">
                  Page {{ box.page }} <span v-if="box.hasDate" class="text-green-600 ml-1">(+Date)</span>
                </div>
              </div>
             </div>
          </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <div class="p-4 border-b flex items-center justify-center gap-4">
            <button @click="goToPrevPage" :disabled="currentViewPage === 1" class="px-4 py-2 bg-blue-600 text-white rounded disabled:bg-gray-300">Previous</button>
            <span class="text-sm font-semibold">Page {{ currentViewPage }} of {{ totalPages }}</span>
            <button @click="goToNextPage" :disabled="currentViewPage === totalPages" class="px-4 py-2 bg-blue-600 text-white rounded disabled:bg-gray-300">Next</button>
          </div>
          ...
          <div class="flex-1 overflow-auto p-6 bg-gray-100">
            ...
            >
              <div 
                class="relative border-2 border-gray-400 rounded shadow-lg bg-white"
              ...
              <template v-for="box in getCurrentPageBoxes()" :key="box.id">
                ...
                <!-- 1. Signature Box -->
                <div
                  class="absolute border-2 rounded transition-all cursor-move group draggable-item"
                ...
                >
                  <div class="absolute -top-6 left-0 bg-purple-600 text-white text-xs px-2 py-1 rounded font-semibold whitespace-nowrap">
                    {{ box.assignedTo }}
                  </div>

                  <!-- Resize Handles -->
                  <div class="absolute w-3 h-3 bg-blue-500 rounded-full -top-1 -left-1 cursor-nw-resize opacity-0 group-hover:opacity-100" @mousedown.stop="startResizing($event, box.id, 'nw')"></div>
                  <div class="absolute w-3 h-3 bg-blue-500 rounded-full -top-1 -right-1 cursor-ne-resize opacity-0 group-hover:opacity-100" @mousedown.stop="startResizing($event, box.id, 'ne')"></div>
                  <div class="absolute w-3 h-3 bg-blue-500 rounded-full -bottom-1 -left-1 cursor-sw-resize opacity-0 group-hover:opacity-100" @mousedown.stop="startResizing($event, box.id, 'sw')"></div>
                  <div class="absolute w-3 h-3 bg-blue-500 rounded-full -bottom-1 -right-1 cursor-se-resize opacity-0 group-hover:opacity-100" @mousedown.stop="startResizing($event, box.id, 'se')"></div>

                  <div class="flex items-center justify-center h-full">
                    <span class="text-xs font-bold text-purple-700 opacity-50">SIGNATURE</span>
                  </div>
                </div>
      ...
      <div class="flex items-center justify-between p-4 border-t bg-gray-50">
        ...
        <div class="flex gap-3">
          ...
          <button @click="saveSignatures" class="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 font-semibold">
            Save Output
          </button>
        </div>
      </div>
