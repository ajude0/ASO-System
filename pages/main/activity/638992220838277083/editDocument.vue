...
const sharedUsersOriginal = ref([]);
const showShareModal = ref(false);
const shareApproverIndex = ref(-1);
const shareScrollContainer = ref(null);

const sharedUsersDirty = computed(() => {
  const a = sharedUsers.value;
  const b = sharedUsersOriginal.value;
  if (a.length !== b.length) return true;
  return a.some((u, i) => {
    const o = b[i];
    return !o || u.emplId !== o.emplId || u.fullAccess !== o.fullAccess;
  });
});
...
const saveSharedUsers = async () => {
  ...
  try {
    const body = sharedUsers.value.map((u) => ({
      emplid: u.emplId || "",
      fullAccess: u.fullAccess ? 1 : 0,
    }));
    ...
    sharedUsersOriginal.value = JSON.parse(JSON.stringify(sharedUsers.value));
    await $swal.fire({
...
const sortedSignatureStatuses = computed(() => {
  ...
  if (sequential.value) {
    return list.sort((a, b) => a.minApprovalOrder - b.minApprovalOrder);
  } else {
...
const getBlockingSigners = (waitingEmplId) => {
  ...
  prePlacedSignatures.value.forEach((s) => {
    if (s.assignedEmplId !== waitingEmplId && s.approvalOrder < minWaitingOrder && s.isEmpty) {
  ...
  return Object.values(blockers).sort((a, b) => a.approvalOrder - b.approvalOrder);
};
  ...
  signatures.forEach((sig) => {
    ...
    if (!grouped[key]) {
      grouped[key] = {
        ...
        pending: 0,
        waiting: 0,
        minApprovalOrder: sig.approvalOrder,
      };
    }
    grouped[key].total++;
...
onMounted(async () => {
  ...
  console.log("docid", originalDocumentid);
  ...
  const fetched = (await fetchSharedUsers(documentid)).map((u) => ({
    ...u,
    fullAccess: u.fullAccess === 1 || u.fullAccess === true,
  }));
  sharedUsers.value = fetched;
  sharedUsersOriginal.value = JSON.parse(JSON.stringify(fetched));
            ...
            >
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p class="text-[11px] transition-colors duration-300 leading-none" :class="isLiveView ? 'text-blue-500' : 'text-zinc-400'">
          ...
          <div v-if="isCancelled == false" class="bg-white rounded-lg shadow-md p-6">
            ...
            <div v-if="sharedUsers.length > 0" class="mt-4 space-y-3">
              ...
              >
                <div class="flex items-center gap-3 px-3 py-3 bg-white">
                  ...
                  <button @click.stop="removeSharedUser(index)" class="hover:text-red-600 transition leading-none font-bold">✕</button>
                </div>
                <!-- Info note for Read Only -->
                <div
                  v-if="!u.fullAccess"
                ...
                >
                  ...
                  <span>Visible to this person only after <strong>all signers have signed</strong>.</span>
                </div>
              </div>
            ...
            <div v-else class="mt-4 flex flex-col items-center py-6 text-gray-300">
              <svg class="w-10 h-10 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a4 4 0 00-5.356-3.712M9 20H4v-2a4 4 0 015.356-3.712M15 7a4 4 0 11-8 0 4 4 0 018 0zm6 3a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
             ...
             <div class="flex justify-end mt-3">
                <button v-if="sharedUsersDirty" class="px-5 py-2 bg-blue-600 text-white rounded-lg font-semibold text-sm hover:bg-blue-700 transition" @click="saveSharedUsers"> Save</button>
              </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              <button
                ...
                :class="isCancelled ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'"
              >
                <svg v-if="!isCancelled" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              ...
              This document has been cancelled and is no longer accepting signatures.
            </div>
          </div>
        </div>
                ...
                <div class="flex items-start gap-2.5">
                  ...
                  >
                    <svg
                      ...
                      :class="{
                        'text-green-700': sig.pending === 0 && sig.waiting === 0,
                        'text-blue-700': sig.pending > 0,
                        'text-gray-400': sig.waiting > 0 && sig.pending === 0,
                      }"
                    >
                      <path v-if="sig.pending === 0 && sig.waiting === 0" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      <path v-else-if="sig.pending > 0" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                      <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="font-semibold text-gray-800 truncate text-sm">{{ sig.assignedTo }}</p>
                    <template v-if="sig.pending === 0 && sig.waiting === 0">
                      <p class="text-xs text-green-700 font-medium mt-0.5">All done — signed everything</p>
                    </template>
                    <template v-else-if="sig.pending > 0 && isCancelled == false">
                      <p class="text-xs text-blue-700 font-medium mt-0.5">
                        Action needed — {{ sig.pending }} pending signature<span v-if="sig.pending > 1">s</span>
                      </p>
                      <div v-if="sig.signed > 0" class="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        <span class="text-xs font-medium bg-green-100 text-green-700 px-2 py-0.5 rounded-full whitespace-nowrap">
                          {{ sig.signed }} signed
                        </span>
                      </div>
                    </template>
                    <template v-else-if="sig.waiting > 0 && isCancelled == false">
                      <p class="text-xs text-gray-400 mt-0.5">
                        Signing after
                        <template v-for="(b, i) in getBlockingSigners(sig.assignedEmplId)" :key="b.assignedEmplId">
                          <span class="font-medium text-gray-600">{{ b.assignedTo.split(" ")[0] }}</span>
                          <span v-if="i < getBlockingSigners(sig.assignedEmplId).length - 2">, </span>
                          <span v-else-if="i === getBlockingSigners(sig.assignedEmplId).length - 2"> and </span>
                        </template>
                      </p>
                    </template>
                  </div>
                  <div class="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span v-if="sig.pending === 0 && sig.waiting === 0" class="text-xs text-green-400 bg-green-100 px-2 py-0.5 rounded-full">Done</span>
                    <span v-else-if="sig.waiting > 0" class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                      {{ isCancelled == false ? 'Waiting' : 'Cancelled' }}
                    </span>
                    <div v-if="sig.pending > 0">
                      ...
                      <span v-else class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full whitespace-nowrap">Cancelled</span>
                    </div>
                  </div>
                </div>
              </div>
    ...
    >
      <div class="w-full max-w-4xl bg-white shadow-lg rounded-2xl p-6 relative">
        ...
        <div class="mt-4 flex justify-end gap-2">
          ...
          >
            Apply — {{ sharedUsers.length }} selected
          </button>
        </div>
      </div>
    </div>
