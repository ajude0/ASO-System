...
import { fetchDocumentTitle, isFreeSign, title,isCancelled } from "~/js/fetchDocumentTitle";
...
const createSignature = async () => {
  const { value: result, isConfirmed } = await $swal.fire({
    ...
    didOpen: () => {
      ...
      const ctx = canvas.getContext('2d');
      ctx.scale(dpr, dpr);       // everything drawn in CSS px
      ctx.lineCap   = 'round';
      ctx.lineJoin  = 'round';
      ctx.lineWidth = 3;
      ...
      // ── FIX: startDraw — record start position and draw initial dot ──
      const startDraw = (e) => {
        e.preventDefault();
        drawing = true;
        const { x, y } = getPos(e);
        lastX = x;
        lastY = y;
        // Draw a dot at the tap/click point so single taps are visible
        ctx.beginPath();
      ...
      // ── FIX: draw — corrected quadraticCurveTo argument order ──
      const draw = (e) => {
        if (!drawing) return;
        e.preventDefault();
        const { x, y } = getPos(e);
        ...
        // FIXED: control point is (midX, midY), endpoint is (x, y)
        // Previously was quadraticCurveTo(lastX, lastY, midX, midY) — wrong!
      ...
      // ── FIX: stopDraw — use lastX/lastY instead of unreliable event coords ──
      const stopDraw = (e) => {
        if (!drawing) return;
        e.preventDefault();
        drawing = false;
        // Use last known position — event coords on mouseup/touchend can be unreliable
        ctx.beginPath();
        ctx.arc(lastX, lastY, penWidth / 4, 0, Math.PI * 2);
        ctx.fillStyle = penColor;
        ctx.fill();
      };
      ...
      thickSlider.addEventListener('input', e => { penWidth = parseInt(e.target.value); });
      ...
      colorBtns.forEach(btn => {
        ...
        btn.addEventListener('mouseleave', () => {
          if (btn.dataset.color !== penColor) btn.style.transform = 'scale(1)';
        });
      });
    ...
    preConfirm: () => {
      ...
      if (!agree.checked) { $swal.showValidationMessage('Please agree to the Electronic Signature Terms & Conditions.'); return false; }
      if (sigType === 'draw') {
        if (!window._sigCanvas || window._sigCanvas.getIsEmpty()) { $swal.showValidationMessage('Please draw your signature first.'); return false; }
        return { type: 'draw', data: window._sigCanvas.canvas.toDataURL('image/png') };
      }
      if (!uploadInput.files.length) { $swal.showValidationMessage('Please upload a signature image.'); return false; }
