<template>
    ...
    <div class="mb-6 p-6">
        <h1 class="text-2xl font-medium text-gray-900">Signed Document Reports</h1>
        <p class="text-sm text-gray-500 mt-1">Overview of all signed documents submitted in the system.</p>
    </div>
        ...
        <div class="flex flex-col md:flex-row justify-between">
            <div class="flex gap-2">
                <div class="relative text-gray-500 focus-within:text-gray-900 mb-4">
                    ...
                    <!-- Input Field -->
                    <input type="text" id="default-search" v-model="query.Search"
                        @keydown.enter="getListOfSignedDocumentReports"
                ...
                <button @click="getListOfSignedDocumentReports"
                            ...
                            <table v-if="!loading" class="table-auto min-w-full rounded-xl">
                                    ...
                                    <tr class="bg-gray-50">
                                        ...
                                        <th @click="sortBy('Document')"
                                            ...
                                            <div class="flex items-center gap-1">
                                                <span>Document</span>
                                                <SortIcon :active="query.SortBy === 'Document'"
                                                    :descending="query.IsDescending" />
                                            </div>
                                        </th>
                                        <th @click="sortBy('SignDate')"
                                            class="p-5 text-left text-sm font-semibold text-gray-900 capitalize cursor-pointer select-none">
                                            <div class="flex items-center gap-1">
                                                <span>Sign Date</span>
                                                <SortIcon :active="query.SortBy === 'SignDate'"
                                                    :descending="query.IsDescending" />
                                            </div>
                                        </th>

                                        <th @click="sortBy('CreatedBy')"
                                            class="p-5 text-left text-sm font-semibold text-gray-900 capitalize cursor-pointer select-none">
                                            <div class="flex items-center gap-1">
                                                <span>Created By</span>
                                                <SortIcon :active="query.SortBy === 'CreatedBy'"
                                                    :descending="query.IsDescending" />
                                            </div>
                                        </th>
                                ...
                                <tbody class="divide-y divide-gray-300">
                                    <tr v-for="(form, index) in forms" :key="index"
                                        ...
                                        <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                            {{
                                                new Date(form.signDate).toLocaleString("en-US", {
                                                    year: "numeric",
                                                    month: "long",
                                                    day: "numeric",
                                                    hour: "numeric",
    ...
    <ViewSignatureBoxPlacement :isOpen="isViewModalOpen" :pdfFile="pdfFile" :signatures="prePlacedSignatures"
        :currentUserEmplId="currentEmplId" @close="isViewModalOpen = false" />
</template>

<script setup>
import {
    getListOfSignedDocumentReports,
    generatePagination,
    changePage,
    totalEntries,
    totalPages,
...
async function viewDocument(id) {
    ...
    await getspecificsignaturepositons(id, currentEmplId.value, false);
