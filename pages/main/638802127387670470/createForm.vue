<template>
  ...
  <div class="max-w-full m-2 mx-auto p-6 bg-white shadow-lg rounded-lg">
    ...
    <div class="mb-4">
      <label class="block font-medium" :class="{ 'text-red-500': errors.title }">Title <span
          class="text-red-500 text-sm"> * </span></label>
      <input v-model="form.title" type="text" maxlength="40" class="border p-2 w-full rounded"
    ...
    <div class="mb-4">
      <label class="block font-medium" :class="{ 'text-red-500': errors.description }">Description <span
          class="text-red-500 text-sm"> * </span></label>
      <textarea v-model="form.description" class="border p-2 w-full rounded" maxlength="240"
    ...
    <!-- Form Objects -->
    <div>
      <draggable v-model="form.formObjects" item-key="id" handle=".drag-handle" ghost-class="drag-ghost"
        chosen-class="drag-chosen" class="space-y-6">
        <template #item="{ element: formObject, index }">
          <div class="mb-6 p-4 border rounded bg-gray-50">
            <div class="flex justify-between items-center mb-4">
              <span class="cursor-move drag-handle text-gray-500 hover:text-gray-700">
                ⇅ Drag
              </span>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div class="col-span-1 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.label }">
                  Label <span class="text-red-500 text-sm"> * </span>
                </label>

                <textarea v-if="
                  formObject.objectType && formObject.objectType === 'LABEL'
                " v-model="formObject.label" type="text" maxlength="255" class="border p-2 w-full rounded"
                  :class="{ 'border-red-500': errors[index]?.label }">
                </textarea>
                <input v-else v-model="formObject.label" type="text" maxlength="255" class="border p-2 w-full rounded"
                  :class="{ 'border-red-500': errors[index]?.label }" />
              ...
              <!-- Select Field -->
              <div class="col-span-1 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.objectType }">
                  Select Field <span class="text-red-500 text-sm"> * </span>
                </label>
                <select v-model="formObject.objectType" class="border p-2 w-full rounded"
                  :class="{ 'border-red-500': errors[index]?.objectType }">
                  <option value="" disabled>Select Field</option>
                  <option v-for="(dropdown, index) in dropdowns" :key="index" :value="dropdown">
                    {{ dropdown }}
                  </option>
                </select>
              ...
              " class="col-span-1">
                ...
                <div v-for="(value, vIndex) in formObject.formObjectLists[0]
                  .values" :key="vIndex" class="flex items-center gap-2 mb-2">
                  <input v-model="formObject.formObjectLists[0].values[vIndex]" type="text" maxlength="80"
                    class="border p-2 w-full rounded" style="text-transform: uppercase" />
                  <button @click="
                ...
                <div class="flex justify-start">
                  <button @click="addFormObjectListValue(index)"
              ...
              " class="col-span-1 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.data }">
                  Data <span class="text-red-500 text-sm"> * </span>
                </label>
                <input v-model="formObject.data" type="text" maxlength="255" class="border p-2 w-full rounded"
              ...
              <div v-if="
                ...
                formObject.objectType == 'DYNAMICSIGNATORY'
              " class="col-span-1 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.display }">
                  Display <span class="text-red-500 text-sm"> * </span>
                </label>
                <input v-model="formObject.display" type="text" maxlength="255" class="border p-2 w-full rounded"
                  @keydown.space.prevent @input="
                    () =>
                ...
                <p v-if="errors[index]?.display" class="text-red-500 text-sm mt-1">
              ...
              ">
                ...
                <select v-model="formObject.isRequired" class="border p-2 w-full rounded">
              ...
              " class="col-span-1 relative">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.variable }">
                  Variable <span class="text-red-500 text-sm"> * </span>
                </label>

                <!-- Trigger input (mimics select) -->
                <div class="border p-2 w-full rounded cursor-pointer bg-white"
                  :class="{ 'border-red-500': errors[index]?.variable }" @click="toggleDropdown(index)">
                ...
                <div v-show="dropdownOpenIndex === index"
                  ...
                  <!-- Filtered options -->
                  <div v-for="(symbol, i) in filteredSymbols(index)" :key="i" @click="selectSymbol(index, symbol)"
                    class="p-2 hover:bg-blue-100 cursor-pointer text-sm">
                    {{ symbol }}
                  </div>

                  <!-- Empty state -->
                  <div v-if="filteredSymbols(index).length === 0" class="p-2 text-gray-500 text-sm">
                    No results found
                  </div>
                </div>

                <!-- Error -->
                <p v-if="errors[index]?.variable" class="text-red-500 text-sm mt-1">
              ...
              " class="col-span-6 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.datasourcescript }">
                  Datasourcescript <span class="text-red-500 text-sm"> * </span>
                </label>
                <textarea v-model="formObject.datasourcescript" type="text" maxlength="255"
                  class="border p-2 w-full rounded" :class="{ 'border-red-500': errors[index]?.datasourcescript }">
                </textarea>
                <p v-if="errors[index]?.datasourcescript" class="text-red-500 text-sm mt-1">
              ...
              " class="col-span-2 mt-1">
                <div class="flex items-center space-x-2">
                  <input type="checkbox" id="approvers-checkbox" class="scale-150 accent-blue-600"
                    v-model="formObject.autofilluser" />
              ...
              <div v-if="formObject.objectType === 'LINKTOOBJECT'" class="col-span-1 mt-1">
                ...
                <select v-model="formObject.textfromsourcelabel" class="border p-2 w-full rounded" :class="{
                  'border-red-500': errors[index]?.textfromsourcelabel,
                }">
                  <option value="" disabled>Select Label</option>
                  <option v-for="(label, index) in textFromSourceLabels" :key="index" :value="label">
                    {{ label }}
                  </option>
                </select>
                <p v-if="errors[index]?.textfromsourcelabel" class="text-red-500 text-sm mt-1">
              ...
              <!-- Columnvalue -->
              <div v-if="formObject.objectType === 'LINKTOOBJECT'" class="col-span-1 mt-1">
                <label class="block font-medium" :class="{ 'text-red-500': errors[index]?.columnvalue }">
                  Columnvalue
                </label>
                <input v-model="formObject.columnvalue" type="text" maxlength="255" class="border p-2 w-full rounded"
                  @keydown.space.prevent @input="
                    () =>
            ...
            <div v-if="form.formObjects.length > 1" class="flex justify-end mt-4">
              <button @click="removeFormObject(index)" class="px-4 py-2 bg-red-600 text-white rounded">
                Remove Section
              </button>
            </div>
          </div>
    ...
    <div class="mb-6 p-4 border rounded bg-gray-50">
      <div class="flex items-center mb-4 space-x-4">
        ...
        <div class="flex items-center space-x-2">
          <input type="checkbox" id="approvers-checkbox" class="scale-150 accent-blue-600" v-model="form.isinorder" />
      ...
      <div class="flex justify-end mb-2 gap-4 mt-4">
        <button v-if="!immediateHeadAdded" @click="addImmediateHead"
          class="px-4 py-2 bg-blue-600 text-white rounded-lg">
          + Immediate Head
        </button>
        <button @click="addApprover" class="px-4 py-2 bg-green-600 text-white rounded-lg">
          + Add Approver
        </button>
      </div>
      <div class="overflow-hidden border rounded-md border-gray-300">
        <div class="overflow-x-auto">
          <table class="table-auto min-w-full rounded-xl">
            ...
            <tbody class="divide-y divide-gray-300">
              <tr v-for="(approver, index) in form.approvers" :key="index"
        ...
        <p v-if="errors.approver" class="flex justify-center text-red-500 text-md mt-2 mb-2">
          {{ errors.approver }}
        </p>
      </div>
    </div>

    <div v-if="form.approvers.length > 0" class="mb-6 p-4 border rounded bg-gray-50">
      <h2 class="font-semibold mb-4">Proxy</h2>
      <div class="flex justify-end mb-2 gap-4 mt-4">
        <button @click="addProxy" class="px-4 py-2 bg-purple-600 text-white rounded-lg">
          + Add Proxy
        </button>
      </div>
      <div class="overflow-hidden border rounded-md border-gray-300">
        <div class="overflow-x-auto">
          <table class="table-auto min-w-full rounded-xl">
            ...
            <tbody class="divide-y divide-gray-300">
              <tr v-for="(proxy, index) in form.proxies" :key="index"
                class="bg-white transition-all duration-500 hover:bg-gray-50">
                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                ...
                <td class="flex p-5 items-center gap-0.5">
                  <button @click="openProxyModal(index)"
                    class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-gray-100 flex item-center">
  ...
  <div v-if="showApproverModal"
    ...
    <div class="w-full max-w-4xl bg-white shadow-lg rounded-lg p-6 relative">
      ...
      <div class="max-h-60 overflow-y-auto border border-gray-300 rounded mt-2" ref="scrollContainer">
        <div tabindex="0">
          <div v-for="(appr, index) in availableApprovers" :key="index" @click="selectApprover(appr)"
  ...
  <div v-if="showProxyModal"
    ...
    <div class="w-full max-w-4xl bg-white shadow-lg rounded-lg p-6 relative">
      ...
      <div class="max-h-60 overflow-y-auto border border-gray-300 rounded mt-2" ref="proxyScrollContainer">
        <div v-for="(appr, index) in availableApprovers" :key="appr.emplId" @click="setSelectedProxy(appr)"
          class="p-2 hover:bg-gray-200 cursor-pointer" :class="{ 'bg-gray-200 font-bold': index === proxyIndex }">
          {{ appr.employeename2 }}
        </div>
        <div v-if="loading" class="p-2 text-gray-400 text-center">
          Loading users...
      ...
      <select v-model="selectedProxyTo" class="border rounded p-2 w-full" @change="setSelectedProxyTo(selectedProxyTo)">
        <option v-for="proxyTo in form.approvers" :key="proxyTo.approverId" :value="proxyTo">
      ...
      <div class="mt-4 flex justify-end gap-2">
        ...
        <button @click="submitProxySelection" class="px-4 py-2 bg-blue-500 text-white rounded-lg disabled:opacity-50">
          Confirm
        </button>
      </div>
    </div>
