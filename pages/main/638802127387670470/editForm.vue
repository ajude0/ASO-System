<template>
  ...
  <div v-else max-w-full m-10 mx-auto p-6 bg-white shadow-lg rounded-lg>
    ...
    <div class="w-full space-y-4 mb-2">
      ...
      <div v-if="!forms.newtemplatefile && oldtemplatefile != null">
        <button
  @click="restoreTemplateFile"
...
>
  Restore Template File
    ...
    <div ref="scrollContainer" class="max-h-full">
      <draggable
          ...
          <div class="p-4 border rounded bg-gray-50">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              >
                ...
                >
                  {{ element.variable || "Select Variable" }}
                </div>

                <!-- Dropdown menu -->
                <div
                  v-show="dropdownOpenIndex === index"
                  class="absolute z-50 bg-white border border-gray-300 w-full mt-1 rounded shadow max-h-60 overflow-y-auto"
                >
                  <!-- Search input -->
                  <input
...
import { getFormObject, forms, loading, atSymbols } from "~/js/fetchFormObject";
import {
  getEmployees,
  availableApprovers,
  query,
...
function handleFileChange(event) {
  ...
  if (file) {
    ...
    oldtemplatefile.value = forms.value.templatefile;
    forms.value.templatefile = null; // Optional: clear old reference
    forms.value.formObjects.forEach((obj) => {
      obj.variable = "";
    });
...
const selectSymbol = (index, symbol) => {
  forms.value.formObjects[index].variable = symbol;
...
const clearExistingFile = async () => {
  const result = await $swal.fire({
    title: "Are you sure?",
    text: "Do you want to remove the existing template file?",
    icon: "warning",
  ...
  if (result.isConfirmed) {
    ...
    await $swal.fire({
      title: "Removed!",
      text: "The template file has been removed.",
      icon: "success",
      timer: 1000,
      showConfirmButton: false,
    });
...
const restoreTemplateFile = async() => {
  if (oldtemplatefile.value) {
    await getFormObject();
    $swal.fire({
      title: "Restored",
      text: "Template file has been restored.",
      icon: "success",
      timer: 1000,
      showConfirmButton: false,
    });
    ...
    $swal.fire({
      ...
      text: "No previous template file was found.",
      icon: "info",
      timer: 1000,
      showConfirmButton: false,
    });
  }
  oldtemplatefile.value = null;
...
const saveFormObjects = async () => {
  ...
  if(forms.value.templatefile){
    formData.append("templatefilepath", forms.value.templatefile);
  }
  ...
  forms.value.formObjects.forEach((obj, i) => {
    ...
    obj.formObjectLists.forEach((listItem, j) => {
      if (listItem.id != null) {
        formData.append(
          `formObjects[${i}].formObjectLists[${j}].id`,
          listItem.id
        );
      }
      ...
      if (listItem.value != null) {
        formData.append(
          `formObjects[${i}].formObjectLists[${j}].value`,
          listItem.value
        );
      }
    });
