...
const handleSaveSignatures = async (boxes) => {
  ...
  sharedUsers.value.forEach((u, i) => {
    form.append(`sharedUsers[${i}].employeeId`, u.emplId || "");
    form.append(`sharedUsers[${i}].name`, u.name);
  });
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mb-1">
              ...
              <input type="text" v-model="pdfTitle" @input="titleError = null" :maxlength="maxlength.Title"
            ...
            <div class="mt-6">
              <div class="flex items-center justify-between p-4 rounded-xl border transition-all duration-300"
                ...
                <button type="button" @click="handleLiveViewToggle"
                  ...
                  :class="isLiveView ? 'bg-blue-600' : 'bg-gray-300'" style="width: 52px;">
                  ...
                  <span class="inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform duration-300 ease-in-out"
                    :style="isLiveView ? 'transform: translateX(28px)' : 'transform: translateX(4px)'"></span>
                </button>
              </div>
            </div>
            ...
            <div class="mt-3">
              <div class="flex items-center justify-between p-4 rounded-xl border transition-all duration-300"
                ...
                <button type="button" @click="handleFreeSignToggle"
                  ...
                  :class="isFreeSign ? 'bg-purple-600' : 'bg-gray-300'" style="width: 52px;">
                  <span class="sr-only">Toggle Free Sign</span>
                  <span class="inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform duration-300 ease-in-out"
                    :style="isFreeSign ? 'transform: translateX(28px)' : 'transform: translateX(4px)'"></span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <p class="text-sm text-gray-600 mb-4">
              Select people who can view this document. They will have read-only access.
            </p>
            ...
            <div v-if="sharedUsers.length > 0" class="mt-4 space-y-2">
              ...
              <div v-for="(u, index) in sharedUsers" :key="u.emplId || u.name"
                ...
                <button @click="removeSharedUser(index)"
                  class="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition flex-shrink-0" title="Remove">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div v-else class="space-y-2 max-h-96 overflow-y-auto">
              <div v-for="(sig, index) in prePlacedSignatures" :key="sig.id" class="p-3 border rounded text-sm"
                :class="{
        ...
        <div v-if="sharedUsers.length > 0" class="mt-4 pt-3 border-t">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="(u, i) in sharedUsers" :key="u.emplId || u.name"
              ...
              {{ u.name.split(',')[0]?.trim() }}
              <button @click.stop="removeSharedUser(i)"
                class="hover:text-red-600 transition leading-none font-bold">✕</button>
            </span>
          </div>
        </div>
